...
import com.tz.vo.CartOrderItemVO;
import com.tz.vo.OrderItemVO;
import com.tz.vo.OrderVO;
import com.tz.vo.ShippingVO;
...
public class OrderService implements IOrderService {
   ...
   // @Transactional//表明该方法加入了事务管理（语句只要抛出异常，之前的语句都会得到回滚）
    @Override
    public ServerResponse createOrder(Integer userId, Integer shippingId) {
        ...
        //2：根据userid查询购物测中已经选中的商品   ->List<Cart>
    ...
    public ServerResponse cancle(Integer userId, Long orderNo) {
        ...
        //3:判断订单状态并取消
        if (order.getStatus()!=Const.OrderStatusdEnum.ORDER_UN_PAY.getCode()){
                return  ServerResponse.serverResponseByError("订单不可取消");
        }


        //4：返回结果
            order.setStatus(Const.OrderStatusdEnum.ORDER_CANCELED.getCode());
    ...
    public ServerResponse get_order_cart_product(Integer userId) {
        ...
        cartOrderItemVO.setImageHost(PropertiesUtils.readKey("imageHost"));
        List<OrderItem> orderItemList= (List<OrderItem>) serverResponse.getData();
        ...
        for (OrderItem orderItem:orderItemList){
            orderItemVOList.add(assembleOderItemVO(orderItem));
        }
    ...
    private OrderVO assembleOrderVO(Order order,List<OrderItem> orderItemList,Integer shippingId){
        ...
        List<OrderItemVO> orderItemVOList=Lists.newArrayList();
       for (OrderItem orderItem:orderItemList){
           OrderItemVO orderItemVO=assembleOderItemVO(orderItem);
       ...
       Shipping shipping=shippingMapper.selectByPrimaryKey(shippingId);
        ...
        Const.OrderStatusdEnum  orderStatusdEnum=  Const.OrderStatusdEnum.codeOf(order.getStatus());
        if (orderStatusdEnum!=null){
            orderVO.setStatusDesc(orderStatusdEnum.getDesc());
        }
        ...
        Const.PaymentdEnum paymentdEnum=Const.PaymentdEnum.codeOf(order.getPaymentType());
        if (paymentdEnum!= null) {
            orderVO.setPaymentTypeDesc(paymentdEnum.getDesc());
        }
    ...
    private OrderItemVO assembleOderItemVO(OrderItem orderItem){
     ...
     * 加库存
     */
    public void addProductStock(Long orderNo){
    ...
    private Order createOrder(Integer userId, Integer shippingId, BigDecimal orderTotalPrice){
        ...
        order.setStatus(Const.OrderStatusdEnum.ORDER_UN_PAY.getCode());
        //订单金额
        order.setPayment(orderTotalPrice);
        order.setPostage(0);
        order.setPaymentType(Const.PaymentdEnum.ONLINE.getCode());
        //保存订单
    ...
    private ServerResponse getCartOrderItem(Integer userId,List<Cart> cartList){
        ...
        for (Cart cart:cartList){
            ...
            if (product==null){
                return  ServerResponse.serverResponseByError("商品id"+cart.getProductId()+"的商品不存在");
            }
            if (product.getStatus()!=Const.ProductStatusEnm.PRODUCT_ONLINE.getCode()){
                return  ServerResponse.serverResponseByError("商品已经下架");
            }
            if (product.getStock()<cart.getQuantity()){
                return ServerResponse.serverResponseByError("库存不足");
            }
    ...
    public ServerResponse alipay_callback(Map<String, String> map) {
          ...
          Order order=orderMapper.findOrderByOrderNo(orderNo);
           if (order==null){
               return  ServerResponse.serverResponseByError("订单"+order+"不是本商品订单");
           }
           if (order.getStatus()>=Const.OrderStatusdEnum.ORDER_PAYED.getCode()){
               //防止支付宝重复调用
           ...
           if (trade_status.equals(Const.TRADE_SUCCESS)){
               //支付成功
               //更改订单状态，更改支付时间
               order.setStatus(Const.OrderStatusdEnum.ORDER_PAYED.getCode());
           ...
           payInfo.setPayPlatform(Const.PaymentdPlatformEnum.ALIPAY.getCode());
    ...
    public void closeOrder(String time) {
      //1:查询订单创建时间小于time的未付款的时间
      List<Order> orderList=orderMapper.findOrderByCreateTime(Const.OrderStatusdEnum.ORDER_UN_PAY.getCode(),time);
      if (orderList!=null&&orderList.size()>0){
          for (Order order:orderList){
              List<OrderItem> orderItemList=orderItemMapper.findOrderItemByOrderno(order.getOrderNo());
              if (orderItemList!=null&&orderItemList.size()>0){
    ...
    public ServerResponse pay(Order order) throws IOException {
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl("http://39.106.1.198:8080/Shopping/order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format("/ftpfile/qr-%s.png", response.getOutTradeNo());
                ...
                map.put("qrCode",PropertiesUtils.readKey("imageHost")+"/qr-"+response.getOutTradeNo()+".png");
